...
/**
 * This class reads in a list of course data and generates an ArrayList that contains the data.
 * 
...
public class MajorModel {
	...
	/**
	 * The constructor initializes the inputFile field. And calls MajorModel's update method.
	 * 
	 * @param inputFileName
	...
	/**
	 * Reads in and parses the data from the file and populates the ArrayList. Depending on if the
	 * recitation check box is clicked the flow is built.
	 * 
		...
		while (bufferedReader.ready()) {
			...
			if (this.recitationCheck == 0) {
				if ((!(courseName.contains("Recitation") || (courseName.contains("Lab"))))) {
					CourseModel course = new CourseModel(subject, courseNumber, courseName,
							prerequisiteCourses, concurrentEnrollment, courseDescription,
							courseEnrollmentInfo, num++);
	 ...
	 * 
	 * @return an ArrayList of CourseModels containing the major's list of courses
	 * @throws Exception
	 */
