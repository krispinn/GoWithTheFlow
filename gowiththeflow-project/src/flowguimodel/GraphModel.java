...
public class GraphModel {
	...
	private ArrayList<CourseModel> listOfCourses = new ArrayList<CourseModel>();    // will store the 'master' CourseModel arraylist
	private Map<String, CourseModel> listOfVertices = new LinkedHashMap<String, CourseModel>();      // this simply creates a hashed array (hash-map) of the above arraylist  
	private Set<DirectedEdgeModel> listOfEdges = new LinkedHashSet<DirectedEdgeModel>();             // stores the links between the courses in the above CourseModel arraylist
...
//		printEdges();
	...
	private void generateEdges() {
		...
		while (it.hasNext()) {
			...
			ArrayList<String> tempPrerequisiteList = current.getPrerequisiteList();         // for that course, save its prerequisite arraylist
 			ArrayList<String> tempConcurrentList = current.getConcurrentList();             // for that course, save its concurrent arraylist
			String courseName;           // we will be using this to index the hash-map (and subsequently connect edges)
				...
				for (int i = 0; i < tempPrerequisiteList.size(); i++) {
					...
					if (listOfVertices.containsKey(courseName)) {                            
						DirectedEdgeModel e = new DirectedEdgeModel(listOfVertices.get(courseName), current,
								listOfVertices.get(courseName).getNum(), current.getNum());                      // create a link between a prereq and a course based on their relative index in the main CourseModel arraylist
						listOfEdges.add(e);        // adds an 'Edge model' to the list. used for drawing/connecting the vertices in the graph. creates a relation between two courses
					}
				}

			} else if (tempConcurrentList != null) {              // the loop is the same as the prereqs
